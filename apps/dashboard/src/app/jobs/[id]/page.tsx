  ...
  return (
    <div className="space-y-1">
      <div className="flex items-start gap-2 flex-wrap">
        <Link
          href="/jobs"
          className="text-xs text-neutral-400 hover:text-neutral-300 transition-colors mt-0.5"
        >
          Jobs
        </Link>
    ...
    <ol className="relative space-y-0">
      ...
      {apps.length === 0 && (
        <TimelineEvent
          ...
          isLast={true}
        >
          <p className="text-xs text-neutral-400">Awaiting action</p>
        </TimelineEvent>
      )}
    ...
    <>
      ...
      {submittedDiffers && (
        <TimelineEvent
          ...
          isLast={isLast()}
        >
          <p className="text-xs text-neutral-400">Formal submission date</p>
        </TimelineEvent>
      )}
...
function AppStatusBadge({ status }: { status: string }) {
  const styles: Record<string, string> = {
    ...
    rejected: "bg-white/[0.04] text-neutral-400",
